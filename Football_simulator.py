...
def plot_match(df):
    ...
    # Convert PIL image to a format Plotly can use
    buffer = io.BytesIO()
...
#    #slider_selection = st.slider('%s' % (message), min_value=min(df['time_batch']), max_value=max(df['time_batch']), value=((0,10)))
...
#    filtered_df = df[(df['game_min'] >= min_time_batch) & (df['game_min'] <= max_time_batch)]
#    return filtered_df
...
def main():
    ...
    if st.session_state.df is None:
        ...
        #st.session_state.df = pd.read_csv(r"C:\Users\Emmanuel.Agholor\Documents\football positions proj\final_data.csv")
...
#     df = pd.read_csv(r"C:\Users\Emmanuel.Agholor\Documents\football positions proj\final_data.csv")
#     print(f'df loaded {pd.Timestamp.now()}')
#     container_1 = st.container()
...
#     st.sidebar.header('Provispo `Visuals`') 
#     min_time_batch, max_time_batch = df_slider_filter('Select Time Range (mins)',df)  
